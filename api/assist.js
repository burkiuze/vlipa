    ...
    if (body.action === 'table') {
      ...
      const answer = await think({
        ...
        system: [
          ...
          'You cannot browse the web, so nobody\'s personal address can be checked from here — but a company\'s',
          'public front-desk address (info@, iletisim@, contact@ at its own domain) follows a pattern, and where',
          'you know the domain you should write it and mark it as the switchboard rather than a named person.',
          'A named person\'s address or today\'s price you do not know: leave those cells empty and put one short',
          'line in note saying which. That is all note is for.',
          'This is never a reason to return fewer rows, and never a reason to return none:',
          'a list of the right companies with the contact column half filled is exactly what was asked for.',
          'Do not refuse, do not explain at length, do not invent an address at a domain you do not know.',
